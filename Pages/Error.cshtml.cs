...
namespace WebApplication1.Pages
{
    ...
    public string? RequestId { get; set; }
        ...
        public ErrorModel(ILogger<ErrorModel> logger, IAuditLogService auditLogService)
        {
_logger = logger;
    ...
    502 => "Bad Gateway Error",
     503 => "Service Unavailable",
        _ => "Unknown Error"
         ...
         string details = $"Status: {statusCode} | Path: {path} | Referer: {referer} | RequestId: {RequestId}";

            await _auditLogService.LogAsync(userId, action, details);
            }
   catch (Exception ex)
       {
            _logger.LogError(ex, "Failed to log error to audit trail for status code {StatusCode}", statusCode);
            }
        }
